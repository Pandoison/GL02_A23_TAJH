...
function getFileNameOf(question) {
    ...
    let fileName = ''
    files.forEach(file => {
        const data = fs.readFileSync(`../data/gift/${file}`, {encoding: 'utf8', flag: 'r'})
...
let editerQuestion = () => {
    ...
    const idQuestion = getIdFromExistingQuestion();
    ...
    switch (typeModificationChoisi){
        ...
        case typeDeModifications[0]:
            ...
            const nouvelIntitule = afficherQuestionReponseLibre('Entrez le nouvel intitule')
            question.consigne = nouvelIntitule
        ...
        case typeDeModifications[3]:
            ...
            console.log(`Voici le cours auquel appartient actuellement la question : ${fileName}`)
            ...
            // parser.deleteQuestionFromGift(question.id)
            // parser.addQuestionToGift()
            break
    }
    listeQuestion = parser.generateQuestion()
}

function getIdFromExistingQuestion() {
    const listeNomQuestion = listeQuestion.map(q => {
        return q.id
    });
    let idQuestion = afficherQuestionReponseLibre("Entrez le nom de la question / HELP pour voir la liste des cours")
...
let supprimerQuestion = () => {
    // =============== Sélection de la question ===============
    let idQuestion = getIdFromExistingQuestion();
